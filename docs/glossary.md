...
이 문서는 Gori에서 사용하는 핵심 용어를 정의합니다.  
프로젝트 소개 문서, UI 문서, 아키텍처 문서, 코드 구현에서 동일한 개념을 같은 이름으로 사용하기 위한 기준 문서입니다.

Gori는 파일, 심볼, export, API, 런타임 이벤트, 그래프 뷰가 서로 얽히는 구조를 가지므로, 용어가 흔들리면 설계와 구현이 빠르게 불안정해집니다.  
이 문서는 그 기준점을 제공합니다.
...
코드베이스의 실제 파일 단위입니다.  
Gori에서 초기 캔버스의 기본 노드는 `File`입니다.
