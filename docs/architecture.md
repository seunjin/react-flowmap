...
route와 layout은 여전히 제품의 주인공이라기보다 **현재 화면 구조의 시작점**을 알려주는 노드입니다.

Next.js App Router의 static route file은 DOM instance가 없기 때문에 synthetic id를 사용합니다.

예:

- `route:src/app/products/page.tsx`

이 synthetic id는 단일 그래프 안에서 route / static / live 노드를 같은 selection 모델로 다루기 위한 내부 키입니다.
...
route / layout node도 `route:<filePath>` synthetic id를 통해 같은 selection 체계 안에 들어옵니다.

이 덕분에 route / static / live ownership graph를 한 workspace 안에서 같은 방식으로 다룰 수 있습니다.
