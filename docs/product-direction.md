...
React Flowmap은 **현재 화면의 프론트엔드 UI 구조를 시각화하고, 각 UI 조각이 어떤 컴포넌트 / 파일 / route context / parent-child tree로 조립되는지 보여주는 개발용 구조 검사기**입니다.
...
- 이 UI를 바꾸려면 어떤 파일을 열어야 하는가
- 이 UI는 어떤 부모/자식 컴포넌트 조합으로 만들어졌는가
- 현재 화면은 어느 route / layout context 아래에서 만들어졌는가
...
3. 이 조각은 현재 화면의 parent-child tree 안에서 어디에 놓여 있는가
4. 현재 화면은 어떤 컴포넌트 구조로 조립되어 있는가
5. 현재 라우트는 어느 layout / page에서 시작되고 있는가
6. 이 구조를 보고 어떤 부분을 더 컴포넌트화할 수 있는가
...
- dev-only screen-to-code structure inspector
- 현재 화면 기준의 component ownership explorer
- 현재 화면을 구성하는 파일 / route / layout context를 연결하는 visual tool
...
React Flowmap의 사용 시나리오는 하나로 제한되지 않습니다.

- 새 프로젝트나 큰 코드베이스 온보딩
- 특정 화면을 수정하기 전에 UI와 파일 구조 파악
- 리팩터링 전 component boundary와 영향 범위 확인
- 디자인과 구현 구조가 어긋난 지점 확인
- 자동 생성되거나 외부에서 수정된 UI 코드의 구조 검증

이 시나리오들은 모두 같은 제품 중심을 공유합니다.

```text
화면 -> 컴포넌트 -> 파일 -> route/layout context -> 현재 트리 구조
```
...
1. 화면에서 UI 조각을 선택한다.
2. 선택한 조각의 owning component와 source file을 확인한다.
3. 필요한 경우 live props와 TypeScript type metadata를 읽는다.
4. 그 조각이 현재 화면 구조 안에서 어디에 놓여 있는지 본다.
5. 전체 graph / file tree로 넓은 문맥을 확인한다.
...
- "이 UI를 바꾸려면 어느 파일을 열어야 하는지 바로 알겠다"
