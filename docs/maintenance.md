# Gori Maintenance Guide

## Purpose

이 문서는 Gori를 장기적으로 유지보수 가능한 프로젝트로 만들기 위한 운영 원칙을 정의합니다.  
초기 단계의 프로젝트는 구현 속도에 밀려 구조가 쉽게 무너질 수 있으므로, 코드보다 먼저 유지보수 기준을 정해둘 필요가 있습니다.

Gori의 유지보수 핵심은 다음 세 가지입니다.
...
Gori의 데이터 구조는 이후 진화할 수 있으므로, 장기적으로는 schema version 개념을 두는 것이 좋습니다.
...
Gori를 유지보수 가능하게 만드는 핵심은 복잡성을 줄이는 것이 아니라, **복잡성이 어디에 있어야 하는지 통제하는 것**입니다.
...
이 문서는 Gori가 초기 프로토타입에서 장기 유지 가능한 라이브러리로 성장하기 위한 운영 기준으로 사용합니다.
