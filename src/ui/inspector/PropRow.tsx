    ...
    <div className="flex flex-col gap-0.5">
      {Object.entries(fields).map(([fname, field]) => (
        <div key={fname} style={{ paddingLeft: depth * 10 }}>
          <div>
            <span className="font-mono text-[10px] text-gori-text-700 font-medium">{fname}{field.optional ? '?' : ''}</span>
            <span className="font-mono text-[10px] text-gori-text-400">: </span>
            {field.fields
              ? <span className="font-mono text-[10px] text-gori-text-400">{'{'}</span>
              : <span className="font-mono text-[10px] text-gori-text-500">{field.type}</span>
            }
          </div>
          {field.fields && (
            <>
              <TypeFieldsView fields={field.fields} depth={depth + 1} />
              <span className="font-mono text-[10px] text-gori-text-400" style={{ paddingLeft: depth * 10 }}>{'}'}</span>
            </>
          )}
        </div>
      ))}
  ...
  return (
    <div className="rounded-[5px] border border-[rgba(229,231,235,0.7)] overflow-hidden bg-[rgba(255,255,255,0.5)]">
      ...
      <div className="flex items-center gap-1 px-2 py-1 bg-[rgba(243,244,246,0.7)] border-b border-[rgba(229,231,235,0.6)]">
        <span className="font-mono text-[11px] text-gori-text-900 font-semibold">{name}</span>
        {typeEntry?.optional && <span className="font-mono text-[11px] text-gori-text-400">?</span>}
        {typeName && (
          <>
            <span className="font-mono text-[10px] text-gori-text-400">:</span>
            {hasTypeDef ? (
              <button
                ...
                className={`font-mono text-[10px] rounded-[3px] px-1 py-0 flex items-center gap-0.5 cursor-pointer transition-all duration-[80ms] ${typeOpen
                  ? 'text-gori-blue bg-[rgba(229,231,235,0.6)] border border-[rgba(203,213,225,0.8)]'
                  : 'text-gori-text-400 bg-transparent border border-transparent'
                  }`}
              >
                <span>{typeName}</span>
              ...
              <span className="font-mono text-[10px] text-gori-text-400">{typeName}</span>
            )}
          </>
        )}
      </div>
      ...
      {typeOpen && (typeEntry?.fields || typeEntry?.resolvedType) && (
        <div className="px-2 pt-1.5 pb-2 border-b border-[rgba(229,231,235,0.6)] bg-[rgba(249,250,251,0.4)]">
          {typeEntry.fields
            ...
            : <span className="font-mono text-[10px] text-gori-text-500">{typeEntry.resolvedType}</span>
          }
        </div>
      )}
      ...
      <div className="px-2 pt-1 pb-1.5">
        {!isExpandable ? (
          ...
          <pre className="m-0 font-mono text-[10px] leading-relaxed text-gori-text-700 whitespace-pre-wrap">
